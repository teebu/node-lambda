...
Lambda.prototype._rsync = function (program, src, dest, callback) {
   var excludes = ['^\.idea', '^\.git', '^.env', '^(.*?)\.log$', '^(.*?)\.swp', '^\.editorconfig', '^deploy.env', '^build', '^.test', '^.test.txt$'],
       excludeGlobs = [];

   if (!program.prebuiltDirectory) { // if prebuiltDirectory is set, we'll allow node_modules
  ...
  cpr.limit = 16;
  var pattern = excludeArgs; // cpr is positive
  //console.log(pattern)
  //(^\.idea$|^.testing$|^.test$|^.git|^.env$|^node_modules|^(.*?)\.log$)
  var re = new RegExp(pattern, "i");
  var options = { filter : re }; // filter out matching files pattern

  cpr(src.trim(), dest, options, function (err) {
    if (err) {
      console.error(err);
      return callback(err);
    }
...
Lambda.prototype._cleanDirectory = function (codeDirectory, callback) {
  fs.emptyDir(codeDirectory, function (err) {
    if (err) {
      throw err;
    }
...
Lambda.prototype._uploadExisting = function(lambda, params, cb) {
  console.log('=>  executing _uploadExisting', params)
  return lambda.updateFunctionCode({
    'FunctionName': params.FunctionName,
    'ZipFile': params.Code.ZipFile,
    'Publish': params.Publish && true // sets Publish to true instead of string 'true'
  }, function(err, data) {
    if(err) {
      return cb(err, data);
    }
  ...
  if (!program.packageDirectory) {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        fs.ensureDirSync(program.packageDirectory);
      } else {
        throw err;
      }
    }
